...
<div id="body-section">
            ...
            <div class="bl-mid">
                ...
                <?php while (have_posts()) : the_post(); ?>
                    ...
                    <div class="cd-detail">
                        ...
                        <p>Arrival Date: <span id="arrival_label"><?php echo ((!empty($cart_detail['arrival_date']) && isset($cart_detail['arrival_date'])) ? date('l, F j, Y', strtotime($cart_detail['arrival_date'])) : (!empty($_SESSION['arrival']) ? date('l, F j, Y', strtotime($_SESSION['arrival'])) : 'N/a')); ?></span><input id="arrival_input" type="text" name="arrival_date" value="<?php echo ((!empty($cart_detail['arrival_date']) && isset($cart_detail['arrival_date'])) ? date('m/d/Y', strtotime($cart_detail['arrival_date'])) : (!empty($_SESSION['arrival']) ? date('m/d/Y', strtotime($_SESSION['arrival'])) : '')); ?>" class="datepicker" style="display: none" onkeypress=""/> <a href="javascript:void(0)" onclick="toggleChange(this, 'arrival_input', 'arrival_label', true)">Change?</a></p>
                        <p>Departure Date: <span id="departure_label"><?php echo ((!empty($cart_detail['depature_date']) && isset($cart_detail['depature_date'])) ? date('l, F j, Y', strtotime($cart_detail['depature_date'])) : (!empty($_SESSION['departure']) ? date('l, F j, Y', strtotime($_SESSION['departure'])) : 'N/a')); ?></span><input id="departure_input" type="text" name="departure_date" value="<?php echo ((!empty($cart_detail['depature_date']) && isset($cart_detail['depature_date'])) ? date('m/d/Y', strtotime($cart_detail['depature_date'])) : (!empty($_SESSION['departure']) ? date('m/d/Y', strtotime($_SESSION['departure'])) : '')); ?>" class="datepicker" style="display: none"/> <a href="javascript:void(0)" onclick="toggleChange(this, 'departure_input', 'departure_label', true)">Change?</a></p>
                        <p>Number of Nights: <span id="nights_label"><?php echo $nights; ?></span></p>
                        <p>Adults: <span id="adults_label"><?php echo (!empty($_SESSION['adults']) ? $_SESSION['adults'] : '1'); ?></span><select id="adults_input" value="<?php echo (!empty($_SESSION['adults']) ? $_SESSION['adults'] : ''); ?>" style="display:none"><option value="0" selected="selected">Select</option><?php
                    for ($i = 1; $i <= $meta['_people'][0]; $i++) {
                        ...
                        ?>
                        <p>Children 3 & up: <span id="children_label"><?php echo (!empty($_SESSION['children']) ? $_SESSION['children'] : 'N/a'); ?></span><select id="children_input" value="<?php echo (!empty($_SESSION['children']) ? $_SESSION['children'] : ''); ?>" style="display:none"/><option value="0" selected="selected">0</option><?php
                            for ($i = 1; $i <= $meta['_people'][0]; $i++) {
                                echo "<option value='$i'>$i</option>";
                            }
    ...
    <!--                        <p>Suite Style: Queen Sofa Suite</p>
                    ...
                    <div class="cd-summary">
                        ...
                        <?php
                        if (!empty($_SESSION['arrival']) && !empty($_SESSION['departure'])) {
                            $price = getCottagePrice(get_the_ID(), $meta['_nightly_rate'][0], $_SESSION['arrival'], $_SESSION['departure'], true);
...
//                            echo "</pre>";
                        ...
                        $service = (get_option("addon_service_charges", 0.00) ? (get_option("addon_service_charges") / 100) : 1.00);
			$price = ($price ? $price : '0.00');
                        ...
                        <div class="row-a">
                            <table style="width: 100%">
                                <tr><td align="left" width="150px"><p>Room</p></td><td align="left">:</td><td align="right" width="*" style="border-width: 0px 0px 1px 0px; border-style: dotted; border-color: #000;">$<span id="rate_room"><?php echo $price; ?></span></td></tr>
...
<script type="text/javascript">
    ...
    jQuery("#add_to_cart").click(function(){
        ...
        var cottage_Id = '<?php echo get_the_ID(); ?>';

        var cottage_title = '<?php echo get_the_title(); ?>';
...
//        var serviceTax = added_price * <?php echo $service; ?>;
//        added_price += serviceTax;
    ...
    function updateTotalPrice(){
        ...
        var rate_tax = (rate_room) * <?php echo $bayview_tax; ?>;
        var service_tax = (rate_pkg) * <?php echo $service; ?>;
        rate_tax = rate_tax + service_tax;
